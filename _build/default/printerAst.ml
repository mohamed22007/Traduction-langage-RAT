...
(* string_of_programme :  programme -> string *)
(* transforme un programme en chaîne de caractère *)
val string_of_programme : A.programme -> string

(* string_of_main :  main -> string *)
(* transforme le main (enum + programme) en chaîne de caractère *)
val string_of_main : A.main -> string

(* print_programme :  main -> unit *)
(* affiche un programme complet *)
val print_programme : A.main -> unit
...
struct
  ...
  let string_of_unaire op =
    ...
    | Numerateur -> "num "
    | Denominateur -> "denom "
    | Address -> "& " (* Affichage de l'adresse *)
  ...
  let rec string_of_affectable a =
    match a with 
      | IdentAffect i -> i^" "
      | PointerAffect a -> "*"^(string_of_affectable a) (* Affichage dereference *)
  ...
  let rec string_of_instruction i =
    ...
    | RetourVoid -> "RetourVoid : RETURN \n"
    | AppelVoid (n,le) -> "AppelVoid : call "^n^"("^((List.fold_right (fun i tq -> (string_of_expression i)^", "^tq) le ""))^")\n"

  (* Conversion des paramètres de fonction *)
  let string_of_param (t, n, is_ref) = 
    let ref_str = if is_ref then "ref " else "" in
    ref_str ^ (string_of_type t) ^ " " ^ n

  (* Conversion des fonctions *)
  let string_of_fonction (Fonction(t,n,lp,li)) = 
    (string_of_type t)^" "^n^" ("^
    ((List.fold_right (fun param tq -> (string_of_param param)^", "^tq) lp ""))^
    ") = \n"^
    ((List.fold_right (fun i tq -> (string_of_instruction i)^tq) li ""))^"\n"

  (* Conversion d'un programme Rat (Fonctions + Bloc principal) *)
  ...
  (* Conversion des énumérateurs *)
  let string_of_enum (Enumerateur(nom, vals)) = 
    "Enum " ^ nom ^ " { " ^ (String.concat ", " vals) ^ " }\n"

  (* Conversion du Main complet (Enum + Programme) *)
  let string_of_main (Main (enums, prog)) =
    (List.fold_right (fun e tq -> (string_of_enum e)^tq) enums "") ^
    (string_of_programme prog)

  (* Affichage d'un programme Rat complet *)
  let print_programme main =
    print_string "AST Syntaxique : \n";
    print_string (string_of_main main);
    flush_all ()
