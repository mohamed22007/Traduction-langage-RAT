...
let string_of_info info =
  ...
  | InfoFun (n,t,tp) -> "Fonction "^n^" : "^(List.fold_right (fun (elt,b) tq -> if tq = "" then (afficher_ref (elt, b)) 
                      else (afficher_ref (elt, b))^" * "^tq) tp "" )^
                      " -> "^(string_of_type t)
  | InfoPoint (n,t,dep,base,point) -> "Pointeur "^n^" : "^(string_of_type t)^" "^(string_of_int dep)^"["^base^"]"^(string_of_int point)
  | InfoEnum (n, dep, taille) -> "Enum "^n^" : "^(string_of_int dep)^"["^(string_of_int taille)^"]"
